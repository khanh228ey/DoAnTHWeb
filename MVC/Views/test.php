...
?>
<head>
    <title>Bootstrap Product Filter</title>
...
<body id="lama-theme" class="index">
    ...
    <main class="mainContent-theme  main-index ">
        <h1 class="hidden entry-title">TÂN THOẠI</h1>

        <div class="form-group">
            <label for="priceRange">Price Range:</label>
            <input type="range" class="form-control-range" id="priceRange" min="0" max="1000000000" step="5">
            <p id="priceLabel">0 - 1000</p>
        </div>
        <div id="home-slider">
                ...
                <div class="item ">
                    <a href="" title="" aria-label="Banner 1">
                        <picture>
                            ...
                            <img src="../../pngtree-cyber-monday-big-sale-at-the-center-on-digital-computer-background-image_446172.png" alt="">
                        </picture>
                    </a>
                </div>
                <!-- <div class="item ">
                    ...
                    <div class="row">
                        <div class="clearfix content-product-list">
                            ...
                            foreach ($result_List as $document) {
                                ...
                                echo ' <div class="product-block product-resize" id="section_one_loop_' . $document['masp'] . '"> ';
                                echo '                   <div class="product-img">';
                                // echo gettype($document['Stock'] );
                                if ((int)$document['soluong'] == (int)0) {
                                    echo '            <div class="product-order">Pre-order</div>';
                                }
                                echo '           <a href="/MVC/controller/productController.php?controller=productDetailId&value=' . $document['masp'] . '" title="' . $document['tensp'] . '" class="image-resize ratiobox">';
                                $callproductService = new productService();
                                $result_images  = $callproductService->findOneImageIdProductIdSort($document['masp']);
                                foreach ($result_images as $item) {
                                    if (!empty($item))
                                        if ($item['mahinh'] == "1") {
                                            echo '               <picture>';
                                            echo '                  <img class="lazyload img-loop" data-sizes="auto" data-src="' . $item['Image'] . '" data-lowsrc="' . $item['Image'] . '" src="' . $item['Image'] . '" alt="' . $document['tensp'] . ' " /> ';
                                            echo '             </picture> ';
                                        }
                                    if ($item['mahinh'] == "2") {
                                        echo '             <picture> ';
                                        echo '                   <img class="img-loop img-hover lazyload" data-src="' . $item['Image'] . '" src="' . $item['Image'] . '" alt=" ' . $document['tensp'] . ' " /> ';
                                        echo '                </picture> ';
                                    }
                                }
                                ...
                                echo '               <span class="pro-price">'.number_format( $document["gia"]  , 0, '.', '.') . 'đ'.'</span> ';
                                echo '           </div> ';
                                echo '       </div> ';
                                echo '       <div class="product-detail clearfix"> ';
                                echo '           <div class="box-pro-detail"> ';
                                echo '              <h3 class="pro-name"> ';
                                echo '                  <a href="/MVC/controller/productController.php?controller=productDetailId&value=' . $document['masp'] . '" title="' . $document['tensp'] . '"> ';
                                echo '                      ' . $document['tensp'] . ' ';
                                echo '                 </a> ';
                                echo '              </h3> ';
                                echo '              <div class="box-pro-prices"> ';
                                echo '                  <p class="pro-price "> ';
                                echo '                       <span>'.number_format( $document["gia"]  , 0, '.', '.') . 'đ'.'</span> ';
                                echo '                  </p> ';
                                echo '              </div> ';
                                echo '           </div> ';
                                echo '      </div> ';
    ...
    <div class="addThis_listSharing ">
        <ul class="addThis_listing">
            ...
            <li class="addThis_item">
                <a class="addThis_item--icon" href="https://www.facebook.com/messages/t/100050420229931" target="_blank" rel="nofollow noreferrer" aria-label="zalo">
